# Install Flow Visual Spec v2.2.0 (as built)

**Status:** SHIPPED — as-built, self-contained.
**Supersedes:** v2.1.1, v2.1 DRAFT. v2.2.0 is the sole authoritative spec for the 31-screen NetBox setup wizard. Earlier specs remain in the repo only as historical snapshots.
**Target:** Shipped on `wiom-tech/wiom-csp-app-apr09@release-01`. Baseline v2.1.1 merged via PR #41 on 2026-04-16; v2.2.0 applies a PRD realignment of the configuration module (§3.13 S16) plus a cross-cutting system-bar inset rule (§0.5).
...
Two corrections land in v2.2.0 after an audit against the Flutter production codebase `installation-supply-view` — the authoritative source of truth for the installation flow. Prior v2.1.x specs had been written against a design-prototype reference repo that drifted from the Flutter PRD on multiple functional dimensions.

1. **§3.13 S16 ISP Form — full PRD realignment.** The v2.1.1 surface had PPPoE-only fields, a nested-in-ISP-branch VLAN picker with the wrong range (128–1492), 5 Static IP fields (including DNS rows that are not in the PRD), and a read-only "Auto" Service Name. v2.2.0 restructures §3.13 end-to-end to match the Flutter PRD:
   - Device ID entry with blocking backend verification — nothing else renders until the device is verified.
   - Branch on device class: router → Connection Type (Fiber / Copper) picker; ONT → VLAN (UNTAG / TAG) + VLAN ID (1–4094) section. VLAN lives at the device-class level, NOT inside the ISP branch.
   - ISP Type radios (PPPoE / Static IP / DHCP), all three branches rendered:
     - PPPoE: Username (3-state inline validator → real backend API), Password, Service Name (editable, optional — validator returns null when blank).
     - Static IP: IP Address, Subnet Mask, Gateway — 3 fields only (no DNS rows) with IPv4 routable-range blocks (127.x, 169.254.x, 224–239.x, 240+) and a cross-field gateway-same-subnet check.
     - DHCP: no input fields; informational strip confirms "no fields needed, tap CTA to continue".
   - CTA "Ready the device" gates on `deviceVerified && connectionTypeOrVlanValid && ispBranchValid`.
   - Backend wiring for `verifyDevice`, `verifyUsername`, `submitIspConfig` is stubbed to `MockInstallationRepository` in the mock flavor for UAT; the real flavor returns `Result.failure(NotImplementedError)` until product managers publish the surface contract. Swap to the real impl in a follow-up PR.
2. **§0.5 system-bar inset invariants (cross-cutting).** `MainActivity` calls `enableEdgeToEdge()` so every surface draws behind the system status bar and nav bar. `WizardTopBar` insets the top, `WizardCtaBar` insets the bottom — but four surfaces end in a `WizardCheckboxRow`, countdown row, or `WizardProgressBar` that sits outside a `WizardCtaBar` and therefore has no built-in nav-bar inset. On gesture-nav / 3-button-nav devices the bottom content clashes with the system nav bar. v2.2.0 adds `Modifier.navigationBarsPadding()` to the root `Column` of those surfaces and formalises the rule. Affected surfaces: S03 PayG Acceptance, S13 Power-Up Timer, S14 Switch-On Confirm, and the shared `TimerAudioCheckScreen` (which backs S17, S20, S23).

No other surfaces, component contracts, tokens, labels, navigation rules, persistence keys, or analytics have changed from v2.1.1 to v2.2.0. §§1–2, §§3.1–3.12, §§3.14–3.33, §4 components, §5 tokens, §§6–9 and (renumbered) §10 are identical in content. §10 drops the resolved "S16 Static IP / DHCP branches not implemented" row and inserts a new "Real backend wiring for S16" deferred item.

## Reading this spec

This document is self-contained. Every surface, component, token, label, navigation rule, and design decision needed to understand or re-implement the NetBox setup wizard is captured inline. No external specs are required. If you are onboarding to the install flow or reviewing a PR touching any wizard screen, this one file tells you everything — there is no "see the DRAFT", "see v2.1.1", or "see v2.0" anywhere that you have to chase.

**Source of truth for the configuration module (§3.13).** The S16 ISP Form behaviour specified here mirrors the Flutter production codebase at `installation-supply-view/lib/modules/tasks/installation/view/start_router_config_view.dart` + `lib/domain/configuration/*.dart` + `lib/common/validators/isp_config_validator.dart`. That Flutter code is the authoritative PRD; this document is its Kotlin-Compose specification. If the two ever diverge, Flutter wins unless a PM has explicitly signed off on a deliberate deviation.
...
### 0.5 System-bar inset invariants (cross-cutting)

`MainActivity.onCreate()` calls `androidx.activity.enableEdgeToEdge()`, so every surface draws behind the system status bar and navigation bar by default. Each wizard surface is responsible for insetting its own chrome so text, icons, CTAs, checkboxes, and progress bars never clash with the status bar at the top or the navigation bar at the bottom.

The shared wizard components handle this for the majority of surfaces:

| Component | Built-in inset | Applies to |
|---|---|---|
| `WizardTopBar` | `.statusBarsPadding()` on its root `Row`, drawn before `.background(brandSecondary)` so the tint still extends behind the status bar | Every screen that has chrome (≈ 29 of 31) |
| `WizardCtaBar` | `.navigationBarsPadding()` on its root `Column`, drawn after `.background(bgScreen)` so the scrolling content above never bleeds through the nav-bar strip | Every screen whose primary advance mechanism is a CTA (≈ 24 of 31) |

The two shared components cover the common case. The remaining surfaces need an explicit per-screen fix.

**Rule — bottom-anchored content without a `WizardCtaBar`.** Any screen whose last child in the root `Column` / `Box` is a `WizardCheckboxRow`, `WizardProgressBar`, countdown row, or any other bottom-anchored composable — and which therefore does **not** sit inside a `WizardCtaBar` — **must** apply `Modifier.navigationBarsPadding()` to the screen's root `Column` (or the outermost `fillMaxSize` container). Without this, the bottom content is eaten by the gesture-pill strip on gesture-nav devices and by the 3-button nav on classic navigation-bar devices.

This applies unconditionally — no "only on some devices" carve-out. Gesture-nav reserves ~32dp at the bottom for the pill; 3-button nav reserves ~48dp; notched devices add a few dp on top; `navigationBarsPadding()` resolves all of these at render time.

**Rule — bottom-anchored content that already lives inside a `WizardCtaBar`.** Do not add `navigationBarsPadding()` to the CtaBar's parent — the CtaBar handles the inset internally.

**Rule — top chrome that is NOT a `WizardTopBar`.** Any screen that forgoes `WizardTopBar` and instead draws its own top affordance (e.g., S13 Power-Up Timer has an agent-focused timer with minimal chrome, S05 Selfie Camera has a full-bleed camera with a floating close button) **must** apply `Modifier.statusBarsPadding()` on the first inset-aware layer so the first element below the status bar does not render under the clock / battery strip.

**Surfaces that require explicit inset padding as of v2.2.0:**

| Surface | Top inset | Bottom inset | Why |
|---|---|---|---|
| S03 PayG Acceptance | `WizardTopBar` built-in | root `Column.navigationBarsPadding()` | `WizardCheckboxRow` is the last child; there is no `WizardCtaBar` below it |
| S05 Selfie Camera | overlay Column `.statusBarsPadding()` | shutter Box `.navigationBarsPadding().padding(bottom = 32.dp)` | full-bleed camera preview; shutter floats over it with its own bottom margin + inset |
| S08 Aadhaar Capture | overlay Column `.statusBarsPadding()` | shutter Box `.navigationBarsPadding().padding(bottom = 32.dp)` | same pattern as S05 |
| S08c PayG System Info | `WizardTopBar` built-in | progress-bar `Box.navigationBarsPadding()` while `!audioComplete`; `WizardCtaBar` built-in inset when audio completes | the `WizardProgressBar` is rendered at the bottom of the Column until audio finishes, before the `WizardCtaBar` reveals — it needs its own inset in the pre-reveal window |
| S13 Power-Up Timer | root `Column.statusBarsPadding()` (no `WizardTopBar`) | root `Column.navigationBarsPadding()` | Agent-focused timer with minimal chrome; body column fills the screen |
| S14 Switch-On Confirm | `WizardTopBar` built-in | root `Column.navigationBarsPadding()` | `WizardCheckboxRow` is the last child; there is no `WizardCtaBar` below it |
| S15 ISP Recharge Audio | `WizardTopBar` built-in | progress-bar `Box.navigationBarsPadding()` while `!audioComplete`; `WizardCtaBar` built-in inset after | same pre-reveal window as S08c |
| S18 NetBox / S21 3-Pin / S24 Wiring cameras (shared `PhotoCaptureScreen`) | `WizardTopBar` built-in over the preview | shutter Box `.navigationBarsPadding().padding(bottom = 32.dp)` | same camera pattern as S05 / S08 |
| S30 Recharge Info Audio | `WizardTopBar` built-in | progress-bar `Box.navigationBarsPadding()` while `!audioComplete`; `WizardCtaBar` built-in inset after | same pre-reveal window as S08c / S15 |
| `TimerAudioCheckScreen` (shared — backs S17, S20, S23) | `WizardTopBar` built-in | root `Column.navigationBarsPadding()` | `WizardProgressBar` / `WizardCheckboxRow` are the last children; there is no `WizardCtaBar` below them |

All remaining surfaces use the `WizardTopBar` + `WizardCtaBar` combo and therefore need no explicit inset modifiers on their own.

**Audio-screen progress-bar pattern.** Every audio briefing (S03 PayG, S08c PayG System Info, S13 Power-Up, S15 ISP Recharge, S17 Placement Check, S20 3-Pin, S23 Wiring, S30 Recharge Info) renders a `WizardProgressBar` at the bottom of the Column while audio plays, and swaps to a `WizardCtaBar` once audio completes. The `WizardCtaBar` applies its own `navigationBarsPadding` — but the `WizardProgressBar` does not. Wrap the `WizardProgressBar` in a `Box(Modifier.navigationBarsPadding())` so the strip clears the system nav bar throughout the full audio window. Skip this when the screen already applies the inset at the root `Column` (S03, S13, S14, shared `TimerAudioCheckScreen`).
...
### 3.13 S16 — ISP Form (configuration module)

**Purpose.** The largest form in the wizard. Collects the full ISP connection configuration — device serial, physical connection class (router Fiber/Copper or fibre ONT with VLAN), ISP credentials — and submits to the backend. On submit, a `WifiConnectDialog` modal prompts the agent to join the NetBox's WiFi network before the wizard advances to S17 Placement Check.

**Source of truth.** This surface mirrors Flutter's `InstallationPPOEConfigView` widget in `installation-supply-view/lib/modules/tasks/installation/view/start_router_config_view.dart`. The three ISP strategies live in `lib/domain/configuration/{config_strategy,ppoe_config,dhcp_strategy}.dart`. Validation rules come from `lib/common/validators/isp_config_validator.dart` and are ported to `core/common/validators/IspConfigValidator.kt` on the Kotlin side. When this spec and the Flutter source diverge, Flutter wins unless a PM has signed off on a deliberate deviation.
...
**Scaffold (always rendered).**
1. `WizardTopBar(title = WiomLabels.pick("ISP डिटेल्स भरें", "Enter ISP details"), onClose = { showExitDialog = true })` — status-bar inset is built-in.
2. Root `Column(fillMaxSize, background = WiomColors.bgScreen)` with a `Modifier.pointerInput(Unit) { detectTapGestures(onTap = { focusManager.clearFocus() }) }` — taps outside an input / radio / CTA clear focus and dismiss the Android soft keyboard. `detectTapGestures` only fires on un-claimed taps, so child clickable elements keep receiving their taps as before. No `.navigationBarsPadding()` at this level because the primary CTA lives inside a `WizardCtaBar` which insets itself (§0.5).
3. Scrolling body `Column(weight = 1f, verticalScroll(rememberScrollState), padding = WiomSpacing.lg, verticalArrangement = Arrangement.spacedBy(WiomSpacing.xxl))`.
4. Title `Text(WiomLabels.pick("ISP डिटेल्स भरें", "Enter ISP details"), 24.sp, Bold, textPrimary)`.
5. §3.13.1 Device ID section (always visible).
6. §3.13.2 Branch-on-device-class section (reveals after §3.13.1 verifies).
7. §3.13.3 ISP Type radios (reveals after §3.13.2 validates).
8. §3.13.4a/b/c ISP branch body (reveals after §3.13.3 picks a type).
9. `state.submitError` inline text if the submit API returns a failure.
10. `WizardCtaBar` with primary CTA "नेट बॉक्स तैयार करें" / "Ready the device" — wrapped in `AnimatedVisibility(visible = ctaEnabled, enter = fadeIn)`.

#### 3.13.1 Device ID entry + verification

**Field.** `SetupInputField(label = "Net Box ID", placeholder = WiomLabels.pick("उदाहरण: 123456789", "e.g. 123456789"), keyboardType = KeyboardType.Number, onValueChange = { digits-only filter })`. Whitespace is stripped on change.

**Trailing 3-state inline validator** (identical chip shape is reused for PPPoE Username in §3.13.4a):
- **IDLE** (text present, not validating, not verified): 28.dp `brandPrimary` circle with white `"→"` 14.sp Bold; tap → starts verification.
- **VALIDATING**: 20.dp `CircularProgressIndicator(brandPrimary, stroke 2.dp)` — simulated via mock or real API round-trip.
- **VERIFIED**: 20.dp `statePositive` circle with white `"✓"` 12.sp Bold; field becomes `readOnly = true`.
- **FAILED**: falls back to the IDLE arrow so the agent can retry; inline error text below the field via `HelperText(text, color = stateNegative)`.

**Editing after verification resets the branch.** If the agent changes the Device ID after a successful verification, the ViewModel clears `deviceVerified`, `inventory`, `connectionType`, `vlanMode`, `vlanId`, and `ispType` — the whole form collapses back to the Device ID row. This prevents a stale device class from haunting downstream branches.

**Mock seeds (UAT).**
| Device ID | Device class | UX path exercised |
|---|---|---|
| `123456789` | ONT | VLAN section appears, then ISP branch |
| `987654321` | ROUTER | Connection Type radios appear, then ISP branch |
| `000000000` | — | Verification fails → inline error, form stays gated |
| anything else | ROUTER | Same as `987654321` |

**API.** `InstallationRepository.verifyDevice(deviceId): Result<InventoryModel>`. Real impl calls the backend; mock seeds the table above with a 1500 ms delay so the three inline states play visibly.

#### 3.13.2 Branch on device class

Rendered only after `deviceVerified == true`. Exactly one of two sub-sections renders, based on `inventory.deviceClass`:

**3.13.2a Router branch (`DeviceClass.ROUTER`).**
A `FieldLabel("Connection Type" / "कनेक्शन टाइप")` followed by two radio rows built with `RadioRow`:
- `Fiber / फाइबर`
- `Copper / कॉपर`

Exactly one must be selected before the ISP Type section (§3.13.3) reveals.

**3.13.2b ONT branch (`DeviceClass.ONT`).**
A `FieldLabel("VLAN")` + `VlanDropdown` with two options: `UNTAG` (default display placeholder) and `TAG`. When the agent picks `TAG`, a VLAN ID field animates in:
- `FieldLabel("VLAN ID (1-4094)")`
- `SetupInputField(keyboardType = KeyboardType.Number, placeholder = WiomLabels.pick("उदाहरण: 128", "e.g. 128"))` — digits-only filter.
- Inline helper / error below the field:
  - On blur with empty value: `"VLAN ID is required for this connection"` (mandatory when TAG).
  - On any non-integer input (dots, commas, letters): `"Enter a whole number between 1 and 4094"`.
  - On parse < 1 or > 4094: `"VLAN ID must be between 1 and 4094"`.

VLAN ID validation comes from `IspConfigValidator.validateVlanId(value, isMandatory = vlanMode == "TAG")`. For UNTAG the VLAN ID is not required and nothing is rendered.

This whole sub-section is absent for router devices; device class is the only switch.

#### 3.13.3 ISP Type radios

Rendered only after §3.13.2 validates (router picked a Connection Type, or ONT picked a VLAN mode with valid VLAN ID if TAG).

A `FieldLabel("ISP टाइप" / "ISP Type")` + three `RadioRow` options:
- `PPPoE` — value `ConnectionType.PPPOE` (0). Agent must pick; no default selection.
- `Static IP` — value `ConnectionType.STATIC_IP` (1).
- `DHCP` — value `ConnectionType.DHCP` (2).

Exactly one selection reveals §3.13.4. Changing the selection does NOT reset entered field values for the other branches — the agent can toggle PPPoE ↔ Static IP and find their earlier entries intact.

#### 3.13.4 ISP branch body

Exactly one of §3.13.4a / 3.13.4b / 3.13.4c renders based on `ispType`.

**3.13.4a PPPoE.** Three fields in order:

1. **Username.**
   - `SetupInputField(label = "Username", placeholder = WiomLabels.pick("ISP username डालें", "Enter ISP username"))`.
   - On-blur validation via `IspConfigValidator.validateUsername(value)`: errors for empty, leading/trailing whitespace, non-printable ASCII, or length > 253.
   - Trailing 3-state inline validator (same chip as §3.13.1 Device ID) tied to a real backend `verifyUsername(username, customerMobile)` API call.
   - On VERIFIED the field goes `readOnly = true`. On subsequent edit, verification state resets to IDLE.
   - If the API returns `exists = true` (username already in use), the field falls back to IDLE and a `UsernameExistsDialog` pops: title "Username पहले से इस्तेमाल में है" / "Username already in use", body "कोई और Username इस्तेमाल करें और दोबारा verify करें." / "Try a different username and verify again.", primary CTA "ठीक है" / "OK". Dismissing clears the username field so the agent can try again.
2. **Password.**
   - `SetupInputField(label = "Password", visualTransformation = PasswordVisualTransformation())` — masked by default.
   - Trailing slot: 👁 / 🙈 text toggle that flips `visualTransformation` between `None` and `PasswordVisualTransformation()`.
   - On-blur validation via `IspConfigValidator.validatePassword(value)`: errors for empty, non-printable ASCII, or length > 253.
3. **Service Name (optional).**
   - `SetupInputField(label = WiomLabels.pick("Service Name (वैकल्पिक)", "Service Name (optional)"), placeholder = WiomLabels.pick("खाली छोड़ सकते हैं", "Leave blank if unsure"))`.
   - **Editable. Optional.** `IspConfigValidator.validateServiceName(value)` returns `null` for blank input. Format errors (whitespace at start/end, non-printable ASCII, length > 253) only surface when the agent actually types something.

**PPPoE branch-complete predicate:** `pppoeUsernameVerified && password non-empty with no error && serviceName has no error`.

**3.13.4b Static IP.** Three fields in order, all shown at once (no progressive reveal inside the branch). Each is an `IpField` (label + `SetupInputField` with `KeyboardType.Decimal` + inline error slot):

1. **IP Address.** Placeholder `WiomLabels.pick("उदाहरण: 192.168.1.1", "e.g. 192.168.1.1")`. Validation via `IspConfigValidator.validateIpAddress`: must parse as dotted-decimal IPv4 with no leading zeros on multi-digit octets, must not be all-zeros or all-ones, must not be in reserved ranges (127.x loopback, 169.254.x link-local, 224–239.x multicast, 240+ reserved).
2. **Subnet Mask.** Placeholder `255.255.255.0`. Validation via `IspConfigValidator.validateSubnetMask`: IPv4 format + cannot be 0.0.0.0 or 255.255.255.255.
3. **Gateway.** Placeholder `WiomLabels.pick("उदाहरण: 192.168.1.254", "e.g. 192.168.1.254")`. Validation via `IspConfigValidator.validateGateway` (IPv4 format + not 0.0.0.0) + the cross-field `validateGatewaySubnet(gateway, ipAddress, subnetMask)` which flags `"Gateway not on same subnet as IP"` when the bitwise `(gw & mask)` does not equal `(ip & mask)`. The cross-field check runs only when all three fields are individually valid; upstream errors win.

**Static IP branch-complete predicate:** all three fields non-empty with no error.

**No DNS fields.** Primary/secondary DNS are NOT part of this surface. The backend provisions DNS server entries centrally.

**3.13.4c DHCP.** No input fields. A short info strip renders inside a rounded `bgInfo` card:

- Leading `Text("ⓘ", 16.sp, stateInfo)`.
- `Text(WiomLabels.pick("DHCP पर कुछ भरने की ज़रूरत नहीं है. सीधे 'नेट बॉक्स तैयार करें' पर टैप करें.", "DHCP needs no extra fields. Tap 'Ready the device' to continue."))`.

**DHCP branch-complete predicate:** `true`.

#### 3.13.5 Primary CTA + submit

- `WizardCtaBar { WiomButton(type = Primary, text = WiomLabels.pick("नेट बॉक्स तैयार करें", "Ready the device"), onClick = { submit() }) }`.
- Wrapped in `AnimatedVisibility(visible = ctaEnabled, enter = fadeIn())`.
- **CTA gate `ctaEnabled`:** `deviceVerified && connectionTypeIsValid && vlanIsValid && ispType != null && ispBranchIsValid && !isSubmitting`. All five gates hold for the CTA to render.
- While `isSubmitting`, the button label flips to "भेजा जा रहा है…" / "Submitting…".
- On tap, the ViewModel assembles the config payload — `PppoeConfig`, `StaticIpConfig`, or neither — together with `vlanId` (only when ONT + TAG) and `connectionType` (only when router) and posts via `InstallationRepository.submitIspConfig(taskId, deviceId, ispType, pppoe, staticIp, vlanId, connectionType)`.
- On success the composable shows `WifiConnectDialog`; the agent confirms "कनेक्ट करें" / "Connect" and the wizard advances to S17.
- On failure `state.submitError` is set and rendered inline above the CTA as 12.sp `stateNegative` text.

#### 3.13.6 WifiConnectDialog

- Outer `Dialog(onDismissRequest = { showWifiDialog = false }, properties = DialogProperties(usePlatformDefaultWidth = false))`.
- Dialog content `Column(padding-horizontal = WiomSpacing.xl, clip RoundedCornerShape(WiomRadius.dialog), background = WiomColors.bgSurface, padding = WiomSpacing.xl, horizontalAlignment = Center)`.
- Icon — `Box(56.dp, CircleShape, WiomColors.bgPositive)` with `Text("📶", 28.sp)`.
- Title — `Text(WiomLabels.pick("WiFi से कनेक्ट करें", "Connect to WiFi"), 20.sp, Bold, textPrimary)`.
- Body — `Text(WiomLabels.pick("नेट बॉक्स के WiFi नेटवर्क से कनेक्ट करें ताकि सेटअप पूरा हो सके", "Connect to the Net Box WiFi network to finish setup"), 14.sp, textSecondary)`.
- SSID row — `Text("SSID: Wiom_NB_${last 6 of deviceId}", 14.sp, SemiBold, textPrimary)`.
- `WiomButton(type = Primary, text = WiomLabels.pick("कनेक्ट करें", "Connect"))` — on tap dismisses the dialog and navigates to S17.

#### 3.13.7 ViewModel / state shape

State lives in `IspFormViewModel` (Hilt-scoped to S16). Key fields in `IspFormUiState`:

```
deviceId, deviceVerification, deviceError, inventory,
connectionType,
vlanMode, vlanId, vlanIdError, vlanDropdownExpanded,
ispType,
pppoeUsername, pppoeUsernameError, pppoeUsernameVerification, pppoeUsernameExistsDialog,
pppoePassword, pppoePasswordError, pppoePasswordVisible,
pppoeServiceName, pppoeServiceNameError,
staticIpAddress, staticIpAddressError,
staticSubnetMask, staticSubnetMaskError,
staticGateway, staticGatewayError,
isSubmitting, submitError, submitSuccess,
```

All mutations go through `IspFormViewModel` action methods (`onDeviceIdChange`, `onVerifyDevice`, `onConnectionTypeChange`, `toggleVlanDropdown`, `onVlanModeSelect`, `onVlanIdChange`, `onVlanIdBlur`, `onIspTypeChange`, `onPppoeUsernameChange`, `onPppoeUsernameBlur`, `onVerifyPppoeUsername`, `dismissPppoeUsernameExistsDialog`, `onPppoePasswordChange`, `onPppoePasswordBlur`, `togglePppoePasswordVisibility`, `onPppoeServiceNameChange`, `onPppoeServiceNameBlur`, `onStaticIpAddressChange`/`Blur`, `onStaticSubnetMaskChange`/`Blur`, `onStaticGatewayChange`/`Blur`, `onSubmitCta`). The composable is a pure function of the collected `StateFlow`.

#### 3.13.8 Label keys

No label keys — this surface uses inline `WiomLabels.pick(hi, en)` at every render site (matching the rest of the wizard). Technical ISP terms (`Username`, `Password`, `Service Name`, `VLAN`, `VLAN ID`, `IP Address`, `Subnet Mask`, `Gateway`, `PPPoE`, `Static IP`, `DHCP`, `Fiber`, `Copper`, `Net Box ID`) stay English in both languages by design — same rule as "WiFi" elsewhere in the wizard.
...
Items intentionally NOT built in v2.2.0 — captured here so reviewers don't file issues for them:
...
v2.2.0 supersedes and **replaces** v2.1.1 (which had superseded the v2.1 DRAFT) as the sole authoritative spec. The v2.1.1 file has been removed from `docs/features/install-flow/` — this single v2.2.0 document is the complete in-tree record of the wizard surface. The v2.1 DRAFT remains in the repo at `docs/features/install-flow/Install_Flow_Visual_Spec_v2.1_DRAFT.md` purely as a historical reference for the original PC audit trail and is not part of any review surface.

Reviewers reading v2.2.0 for the first time should read it top-to-bottom — the document is self-contained and every surface, component, token, label, navigation rule, and design decision needed to understand the wizard is captured inline.
...
3. Check the relevant cross-cutting rules in `§0` (status bar padding, Hindi/English pick(), exit dialog contract, system-bar inset invariants in §0.5)
4. Skim `feature/installation/src/main/java/com/wiom/csp/feature/installation/redesign/screens/` for the mapped `S<N>*.kt` file

The 89+ image/audio/Lottie assets under `feature/installation/src/main/assets/img/` and `feature/installation/src/main/res/raw/` are committed from earlier scaffold passes — they are not new in the v2.2.0 PR.

---

*End of Install_Flow_Visual_Spec_v2.2.0.md.*
